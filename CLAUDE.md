...
- **Table D.1 — robustness.** Null survives alt cutoff, treatment variants, COVID drop, state×year-quarter FE, two-way clustering, telework-eligible-only, the **exact-birthdate treatment-ceiling sweep** (48/49/51/53/60 months), and the **early/late timing split**; carries the **A-vs-B placebo** (licenses Control A as clean). *Why it's needed:* the null is not an artifact of any single design choice, including where the age boundary is drawn.
...
| Table — Robustness (first stage, exact-birthdate ceiling sweep, early/late timing split, log earnings) | `tab07_robustness.tex` | 06 |
