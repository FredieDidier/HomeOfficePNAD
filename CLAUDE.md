...
| `clt_covered` | = 1 if VD4009 ∈ {1,5} (signed-card employees, **private OR public sector**). The **correct "legally covered by Art. 75-F" group**: public-sector celetistas (empresas públicas / sociedades de economia mista — Banco do Brasil, Caixa, Petrobras, Correios) are hired under the CLT and ARE covered. Excludes statutory servants (VD4009==7), domestic (3), informal, self-employed. Canonical in `build/01_pnadc.R`; **also derived on-the-fly from VD4009 at the top of scripts 05/08/09 so no rebuild is needed**. Public CLT ≈ 1.8% of employed treated women, ≈4.8% of the covered pool → null unchanged, but fixes the coverage claim. **This is the group used for all "covered" estimands/heterogeneity/reach.** | Derived |
| `clt_private` | = 1 if VD4009 == 1 (private-sector carteira-assinada). Kept as the **narrower robustness split** only; superseded by `clt_covered` as the headline "law binds here" group. | Derived |
...
### Round 3 (July 2026 — friend's read + second econ-GPT structural report; paper now 53 pp)
Two feedback sets addressed together. **Institutional discovery (load-bearing):** public-sector CLT employees ARE covered by Art. 75-F. Public companies / mixed-economy firms (Banco do Brasil, Caixa, Petrobras, Correios) hire under the CLT, so their `empregados` are celetistas and covered; only *statutory* servants (servidores estatutários, VD4009==7) are outside. Fix = new **`clt_covered` = VD4009 ∈ {1,5}** (private + public signed-card), now the headline "legally covered" group everywhere; **`clt_private` (==1) demoted to a robustness split.** Derived on-the-fly from VD4009 in scripts 05/08/09 (no rebuild) + canonical in `build/01_pnadc.R`. Magnitude: public CLT ≈1.8% of employed treated, ≈4.8% of covered pool → **null unchanged** but the coverage claim is now correct. New numbers: CLT-covered estimand **−1.32pp, CI [−3.52, 0.88]**; CLT×teleworkable **−0.89pp, CI [−6.86, 5.08]** (replace the old [−3.66,0.92] / [−7.25,5.38] everywhere). Two labor-court news items (a Correios worker; a public bank-clerk *escriturária*) — both **public-CLT celetistas** — cited in a §2.1 footnote as illustration (Comment 3: litigation is highly selected, not evidence of take-up/compliance).
- **New exhibit — statutory-reach funnel `tab16_statutory_reach.tex` (Table 3, main text §5.1, built in `08_referee_revision.R`):** weighted funnel among young-child women — 100% → employed 50.2 → salaried 30.8 → **CLT-covered 18.9** → CLT teleworkable 6.1 → not-yet-WFH **5.9**. Descriptive, not causal (`\label{tab:reach}`). This is the reviewer's preferred way to explain the null (vs. more heterogeneities).
- **Structural moves to appendix (Comments 6, 7):** `tab03a` (outcomes A), `tab15` (earnings margins), and `tab08` (triple diff) **moved to appendix** (now §E). Main §5.2 renamed **"Secondary labor-market outcomes"** (short, 2 paras); §5.3 renamed **"Implementation margins: predetermined teleworkability and occupational sorting"** (was "Mechanisms"). DDD reframed as a **female-specific-response test** (added Eq. `eq:ddd`, "not an untreated control"); earnings/hours reframed as **secondary conditional results, NOT clean causal** (employment is post-treatment; flat conditional pre-trends don't fix compositional selection).
- **Language purge (Comment 9):** every "precise/tight/genuine null" → "small and statistically insignificant"; "population-average null is precise" → "informative about large increases … does not establish equivalence at 0.5pp"; covered-group CIs described as too wide for equivalence.
- **Comment 1 (intro):** 2nd paragraph reframed around **access to a non-wage job amenity** (employer-mediated, segmented market); added Mas–Pallais WTP heterogeneity + `lavetti2023` (new ref) on inferring amenity preferences from wage differentials; contribution sentence on "implementability of non-wage amenities in segmented labor markets".
- **Comment 8:** Fig 1 caption notes the post-pandemic elevation is common to groups (absorbed by quarter FE); framing standardized to "limited causal evidence in **middle-income** labor markets" (no "developing countries" over-claim). TST/enforcement/shift-share extensions deliberately **NOT** pursued (reviewer agreed).
- **Mechanical:** abstract opens "Beginning in March 2022"; "Monastério"→**"Monasterio"**; "public servants"→**"statutory civil servants"**; **bibliographystyle `plainnat` → `apalike`** (surname-first for all authors; JPopE-appropriate). Compiles clean, 53 pp, no undefined refs.
- **Doc drift:** the appendix "Additional tables and figures" section is now **§E** (was cited as D in earlier rounds); exhibit map above still uses old D.x / "Table 3 precise zeros" wording — treat the numbers here as authoritative.
